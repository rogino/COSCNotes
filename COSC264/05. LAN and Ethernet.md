...
```
E[Completion Time]    = E[Transmission Delay] + E[Access Time]
                      = E[Transmission Delay] + 0
                      = 1 s
...
However, each station either needs *N* receivers or tunable receivers, frequency synchronization is needed, and there is no re-use.
...
```
E[Access Delay]    = T_SF/2 = 0.5 s (on average)
E[Completion Time] = E[Access Delay] + E[Transmission Delay]
                   = 0.5 s + 1/N s <= 1 s (less than for N > 2)
```
...
A random integer is uniformly generated the range `[0, pow(2, min(10, coll)) - 1]` - this is the **backoff window**, and is multiplied by the **slot time**.
