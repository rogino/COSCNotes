# 9. Introduction to Routing - Distance Vector Algorithm
...
At `t1`, `z` receives an update from `y` and calculates `D_z(x)`:
...
Before the link cost change, `z` lies to `y`, saying `D_z(x) = \infty` as long as it uses `y` to route to `x`. In `y's` DV table:
...
At `t2`, `y`, updates `z`, now without lying. `y` calculates `D_y(x)`:
...
At `t0`, `y` detects `cost(x, y) = 60` and recalculates its vector table: `D_y(x)` is the minimum of:
...
To fix this, **the maximum cost of a path is limited to 15**.
