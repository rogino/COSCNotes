...
Smooth continuity (e.g. smooth curves vs straight lines right angles) easier to perceive, but less 'neat'.
...
Strict statistically testable hypothesis: better/worse or no evidence/difference.

Measure the participants' response to manipulation of experimental conditions.

The results are repeatable (when the methods are defined rigorously), but is also time-consuming and expensive.
-->
