...
If a person has to pick out a particular item out of $n$ items, the time $T$ taken to find the item increases linearly: $O(n)$. However, pop-out effects where one item is visually distinct, reduces this to $O(1)$. However, this requires the interface to predict what the user wants to select.
...
- Intra-modal improvement e.g. guidance techniques where you show items the user is likely to use
- Inter-modal improvement e.g. skillometers
- Vocabulary extension e.g. community command use
- Task strategy: slow transition to command line interface
...
- e.g. searching for something on Wikipedia, ending up on an unrelated page 30 minutes later

Associative activation error:

- Internal interference; brain thinks of some related topic, derailing task
...
Peak effect: people's memories of experiences are influenced by the peak/most intense moments of an experience (e.g. maximum pain more influential than total 'area' of pain).
