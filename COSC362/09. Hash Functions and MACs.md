...
- $H$ takes as input message $m$ of **abitrary length** and outputs a *message digest* $H(m)$ of **fixed length**
...
In comparision, to get a 50% change of guessing the key of a block cipher requires only $2^{k - 1}$ trials, so hash functions require about double the number of bits compared to block ciphers for the same security.
...
The tag $T$ is appended to the message and the recipient can compute $T'$ with the message they receieve and their shared secret, checking to ensure that $T' = T$.
