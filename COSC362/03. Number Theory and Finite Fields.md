...
$$
\begin{align}
a   &= bq_1   + r_1 \textrm{ for } 0 \gt r_1 \gt b   \\
b   &= r_1q_2 + r_2 \textrm{ for } 0 \gt r_2 \gt r_1 \\
r_1 &= r_2q_3 + r_3 \textrm{ for } 0 \gt r_3 \gt r_2 \\

\dots \\

r_{k - 2} &= r_{k - 1}q_k + r_k \textrm{ for } 0 \gt r_k \gt r_{k - 1} \\
r_{k - 1} &= r_kq_{k + 1} \textrm{ with } r_{k + 1} = 0
\end{align}
$$
...
$r_{k - 2} = r_{k - 1}q_k + r_k$ can be rewritten as $r_k = r_{k - 2} - r_{k - 1}q_k$. $r_{k - 1}$ can be substituted with the value above, and this process can be repeated until we get back to the required form.
...
$b$ is a *residue* of $a \bmod n$ if $a - b = kn$ for some integer $k$:

$$
a \equiv b (\bmod n) \Longleftrightarrow a - b = kn
$$

Given $a \equiv b (\bmod n)$ and $c \equiv d (\bmod n)$:
...
$b \bmod n$ denotes the **unique** value $a$ in the complete set of residues $\{ 0, 1, \dots, n-1\}$ such that:

$$
a \equiv b (\bmod n)
$$

In other words, $b \bmod n$ is the remainder after dividing $a$ by $n$.

#### Residue Class

The set $\{ r_0, r_1, \dots, r_{n - 1}\}$ is a *complete* set of residues \bmodulo $n$ if for every $a$, $a \equiv r_i (\bmod n)$ for **exactly one** $r_i$.
...
The inverse if $a$ (if it exists) is a value $x$ such that $ax \equiv 1 (\bmod n)$; it is written as $a^{-1} (\bmod n)$.

Theorem: let $0 \gt a \gt n$; $a$ has an inverse \bmodulo $n$ iff $gcd(a, n) = 1$.

The Euclidean algorithm can be used to find the inverse of $a$.

If $x$ such that $ax \equiv 1 (\bmod n)$, there is an integer $y$ such that $ax = 1 + yn$.

Hence, starting from $gcd(a, n) = 1$, use back substitution to find $x$ and $y$ in the equation $ax + ny = 1$.
...
- $\mathbb{F} \textbackslash \{ 0 \}$ is an abelian group under the operation $\cdot$ with identity element $1$​
