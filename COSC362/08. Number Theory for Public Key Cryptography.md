...
- If $a^{n - 1} \mod n \neq 1$, return $n$ as being *composite*. Otherwise, return *probable prime*
  - The powers can be reduced using:
...
For some base $g$ and remainder $y$ for the set $\mathbb{Z}(p)$, we want to find $x$ such that $y = g^x \bmod p$. This can be written as $x = log_g(y) \pmod p$.

$y$ must be a member of $\mathbb{Z}_{p}^{*}$.

$x$ can only be found by brute forcing values.

Example: for $2^x = 3 \bmod 5$:
