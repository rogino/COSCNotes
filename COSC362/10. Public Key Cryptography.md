...
Both compute the secret key $Z = g^{ab} \bomod p$: $(g^b)^a \equiv (g^a)^b) \pmod p$.
...
Algebraic structures formed from cublic equations.
...
A point on the curve is the identity element, and by defining a binary operation on the points (e.g. multiplication), we can form a group over elliptic curve's points: the eliptic curve group.
