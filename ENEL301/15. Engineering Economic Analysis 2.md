...
We can then use the standard compount interst fomula to find that $\text{Price} = \$200 + frac{\$291.26}{1 + 5\%} + frac{\$282.78}{(1 + 5\%)^2} = \$733.88$.

#### Example: Discount Rate

Government considering new hydroelectric power plant with capital expendature of 100 million in year 1 and increasing at 10% per annum for the next two years (currently no inflation therefore real). What is the present value of the cost in real terms?
